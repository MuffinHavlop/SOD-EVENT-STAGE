...
void loop()
{
  ...
  if (myPS2.ButtonPressed(PSB_PAD_UP)) { // Check if PS2 Select button is pressed
  ...
  if (myPS2.ButtonPressed(PSB_PAD_DOWN)) { // Check if PS2 Select button is pressed
  ...
  if (myPS2.ButtonPressed(PSB_PAD_LEFT)) { // Check if PS2 Select button is pressed
    motorState3 = !motorState3;
    if (motorState3 == true){
      ctrl_dc(MOT_LEFT, (true) ? SPD_TURN : SPD_TURN);
      ctrl_dc(MOT_RIGHT, (true) ? 0 : 0);
    }
    else{
      ctrl_dc(MOT_RIGHT, (true) ? 0 : 0);
  ...
  if (myPS2.ButtonPressed(PSB_PAD_RIGHT)) { // Check if PS2 Select button is pressed
    motorState4 = !motorState4;
    if (motorState4 == true){
      ctrl_dc(MOT_RIGHT, (true) ? SPD_TURN : SPD_TURN);
      ctrl_dc(MOT_LEFT, (true) ? 0 : 0);
